<template>
  ...
  <v-card class="pt-1">
        <v-card-title class="align-end pl-4 pa-2 mt-2 mb-3 rounded white--text elevation-1" style=" margin-left: -8px; margin-right: -8px; max-height: 50px; background: linear-gradient(to right, #0d47a1, #0d47a1, #1A237E);">
                <span class="font-semibold"><v-icon  dark left>mdi-list-box</v-icon>SEMESTERS</span>
            </v-card-title>
    <v-data-table
        :headers="headers"
        :items="table_data"
    ...
    <v-dialog v-model="showDeleteDialog" persistent width="400">
      ...
      <v-card-title class="align-center ma-2 mt-0 pa-2 rounded white--red elevation-0" >
        ...
        <v-card-text>
          ...
          (<i>{{itemToDelete ? 'Code: '+itemToDelete.code+' Description: '+itemToDelete.description : ''}}</i>)
        </v-card-text>
        <v-card-actions>
          <v-spacer/>
    ...
    export default {
      ...
      watch: {
        ...
        'forms.signatory': debounce(function (val) {
          ...
          let pageNumber = page;
           axios
            .get(`/api/v1/getSemesters?page=` + pageNumber,{
              params: { 
      ...
      methods: {
        ...
        async deleteData(val) {
          try {
            await axios.delete('/api/v1/adminsemesters/'+ this.itemToDelete.id, {
