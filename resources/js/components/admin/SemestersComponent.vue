      ...
      <v-col cols="12" lg="3">
        <v-card class=" rounded-medium pt-1 pb-2 px-2"
            ...
            <v-form 
                ...
                ref="form"
              >
              <label class="black--text font-weight-medium" for="">Code</label>
              <v-text-field
                      v-model="forms.code"
                      label=""
                      auto-grow
...
<script>
  import Table from "../../pages/components/SemesterTable";
export default {
  ...
  async mounted() {
    ...
    await axios.get(`/api/v1/adminsemesters`).then((response) => {
    ...
    async submit() {
      if (this.$refs.form.validate()) {
        ...
        if (this.isEditMode) {
          // save changes
          try {
            const response = await axios.put('/api/v1/adminsemesters/'+ this.editedForms.id, data);
            this.editedForms = this.defaultForms;
          ...
          // add item
          try {
            const response = await axios.post('/api/v1/adminsemesters', data);
            this.$refs.childComponent.nextPage();
            this.$refs.childComponent.editRowReset();
