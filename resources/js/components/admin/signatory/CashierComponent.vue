...
export default {
  ...
  methods: {
     readDataFromAPI() {
      ...
      let pageNumber = page;
      axios
        .get(`/api/v1/cashiers?page=` + pageNumber, {
    ...
    searchIt(d) {
      if (d.length > 2) {
        ...
        let pageNumber = page;
        axios
          .get(`/api/v1/cashiers/${d}?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
          .then((res) => {
            this.loading = false;  
    ...
    deleteItem(item) {
      const index = this.cashiers.data.indexOf(item);
      let decide = confirm("Are you sure you want to delete this item?");
      if (decide) {
        axios
          .delete("/api/v1/cashiers/" + item.id)
          .then((res) => {
            this.text = "Record Deleted Successfully!"; 
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            this.cashiers.data.splice(index, 1);
          })
          .catch((err) => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        axios
          .put("/api/v1/cashiers/" + this.editedItem.id, this.editedItem)
          .then((res) => {
            this.text = "Record Updated Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            Object.assign(this.cashiers.data[index], res.data.cashier);
            console.log(this.editedItem);
          })
          .catch((err) => {
            console.log(err.response);
        ...
        axios
          .post("/api/v1/cashiers", this.editedItem)
          .then((res) => {
            this.text = "Record Added Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            // this.cashiers.data.push(res.data.cashier); 
            this.cashiers = res.data.cashiers
          })
          .catch((err) => {
            console.dir(err);
