   ...
   <v-card>
    <v-container>
     <v-data-table
        ...
        :server-items-length="totalOSASs"
        :items-per-page="10" 
        show-select 
        :footer-props="{
                ...
                > 
                    <v-container v-if="editedIndex!=-1">
                      <v-row>
                        <v-col cols="12" sm="12">
                          <v-text-field
                            v-model="editedItem.osas_number"
                            label="OSAS Number"
      ...
      <td>{{osass.data.indexOf(item)+1}}</td> 
    </template>
...
export default {
  ...
  methods: {
     readDataFromAPI() {
      ...
      let pageNumber = page;
      axios
        .get(`/api/v1/osass?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
        .then((response) => {
          //Then injecting the result to datatable parameters.
          this.loading = false;
         this.osass = response.data.osass;
          this.campuses = response.data.campuses;
          this.programs = response.data.programs;
          this.sections = response.data.sections;
          this.totalOSASs = response.data.osass.total;
          this.numberOfPages = response.data.osass.last_page;
        });
    },
    ...
    searchIt(d) {
      if (d.length > 2) {
        ...
        let pageNumber = page;
        axios
          .get(`/api/v1/osass/${d}?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
          .then((res) => {
            this.loading = false;  
            this.osass = res.data.osass;
            console.log(this.osass);
            this.totalOSASs = res.data.osass.total;
            this.numberOfPages = res.data.osass.last_page;
          })
          .catch((err) => {
            console.error(err);
          });
    ...
    deleteItem(item) {
      const index = this.osass.data.indexOf(item);
      let decide = confirm("Are you sure you want to delete this item?");
      if (decide) {
        axios
          .delete("/api/v1/osass/" + item.id)
          .then((res) => {
            this.text = "Record Deleted Successfully!"; 
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            this.osass.data.splice(index, 1);
          })
          .catch((err) => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        axios
          .put("/api/v1/osass/" + this.editedItem.id, this.editedItem)
          .then((res) => {
            this.text = "Record Updated Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            Object.assign(this.osass.data[index], res.data.osas);
            console.log(this.editedItem);
          })
          .catch((err) => {
            console.log(err.response);
        ...
        axios
          .post("/api/v1/osass", this.editedItem)
          .then((res) => {
            this.text = "Record Added Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            // this.osass.data.push(res.data.osas); 
            this.osass = res.data.osass
          })
          .catch((err) => {
            console.dir(err);
