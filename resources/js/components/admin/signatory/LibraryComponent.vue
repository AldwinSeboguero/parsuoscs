   ...
   <v-card>
    <v-container>
     <v-data-table
        ...
        :server-items-length="totalLibrarys"
        :items-per-page="10" 
        show-select 
        :footer-props="{
          itemsPerPageOptions: [5, 10, 15],
          itemsPerPageText: 'Library Per Page',
                ...
                > 
                    <v-container v-if="editedIndex!=-1">
                      <v-row>
                        <v-col cols="12" sm="12">
                          <v-text-field
                            v-model="editedItem.library_number"
      ...
      <td>{{librarys.data.indexOf(item)+1}}</td> 
    </template>
...
export default {
  ...
  methods: {
     readDataFromAPI() {
      ...
      let pageNumber = page;
      axios
        .get(`/api/v1/librarys?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
        .then((response) => {
          //Then injecting the result to datatable parameters.
          this.loading = false;
         this.librarys = response.data.librarys;
          this.campuses = response.data.campuses;
          this.programs = response.data.programs;
          this.sections = response.data.sections;
          this.totalLibrarys = response.data.librarys.total;
          this.numberOfPages = response.data.librarys.last_page;
        });
    },
    ...
    searchIt(d) {
      if (d.length > 2) {
        ...
        let pageNumber = page;
        axios
          .get(`/api/v1/librarys/${d}?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
          .then((res) => {
            this.loading = false;  
            ...
            this.totalLibrarys = res.data.librarys.total;
            this.numberOfPages = res.data.librarys.last_page;
          })
          .catch((err) => {
            console.error(err);
          });
    ...
    deleteItem(item) {
      const index = this.librarys.data.indexOf(item);
      let decide = confirm("Are you sure you want to delete this item?");
      if (decide) {
        axios
          .delete("/api/v1/librarys/" + item.id)
          .then((res) => {
            this.text = "Record Deleted Successfully!"; 
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            this.librarys.data.splice(index, 1);
          })
          .catch((err) => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        axios
          .put("/api/v1/librarys/" + this.editedItem.id, this.editedItem)
          .then((res) => {
            this.text = "Record Updated Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            Object.assign(this.librarys.data[index], res.data.library);
            console.log(this.editedItem);
          })
          .catch((err) => {
            console.log(err.response);
        ...
        axios
          .post("/api/v1/librarys", this.editedItem)
          .then((res) => {
            this.text = "Record Added Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            // this.librarys.data.push(res.data.library); 
            this.librarys = res.data.librarys
          })
          .catch((err) => {
            console.dir(err);
