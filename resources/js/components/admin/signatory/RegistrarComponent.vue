   ...
   <v-card>
    <v-container>
     <v-data-table
        ...
        :items="registrars.data"
        :options.sync="options"
        :server-items-length="totalRegistrars"
        :items-per-page="10" 
        show-select 
        :footer-props="{
          itemsPerPageOptions: [5, 10, 15],
          itemsPerPageText: 'Registrar Staff Per Page',
                ...
                > 
                    <v-container v-if="editedIndex!=-1">
                      <v-row>
                        <v-col cols="12" sm="12">
                          <v-text-field
                            v-model="editedItem.registrar_number"
      ...
      <td>{{registrars.data.indexOf(item)+1}}</td> 
    </template>
...
export default {
  ...
  methods: {
     readDataFromAPI() {
      ...
      let pageNumber = page;
      axios
        .get(`/api/v1/registrars?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
        .then((response) => {
          //Then injecting the result to datatable parameters.
          this.loading = false;
         this.registrars = response.data.registrars;
          this.campuses = response.data.campuses;
          this.programs = response.data.programs;
          this.sections = response.data.sections;
          this.totalRegistrars = response.data.registrars.total;
          this.numberOfPages = response.data.registrars.last_page;
        });
    },
    ...
    searchIt(d) {
      if (d.length > 2) {
        ...
        let pageNumber = page;
        axios
          .get(`/api/v1/registrars/${d}?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
          .then((res) => {
            this.loading = false;  
            this.registrars = res.data.registrars;
            console.log(this.registrars);
            this.totalRegistrars = res.data.registrars.total;
            this.numberOfPages = res.data.registrars.last_page;
          })
          .catch((err) => {
            console.error(err);
          });
    ...
    deleteItem(item) {
      const index = this.registrars.data.indexOf(item);
      let decide = confirm("Are you sure you want to delete this item?");
      if (decide) {
        axios
          .delete("/api/v1/registrars/" + item.id)
          .then((res) => {
            this.text = "Record Deleted Successfully!"; 
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            this.registrars.data.splice(index, 1);
          })
          .catch((err) => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        axios
          .put("/api/v1/registrars/" + this.editedItem.id, this.editedItem)
          .then((res) => {
            this.text = "Record Updated Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            Object.assign(this.registrars.data[index], res.data.registrar);
            console.log(this.editedItem);
          })
          .catch((err) => {
            console.log(err.response);
        ...
        axios
          .post("/api/v1/registrars", this.editedItem)
          .then((res) => {
            this.text = "Record Added Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            // this.registrars.data.push(res.data.registrar); 
            this.registrars = res.data.registrars
          })
          .catch((err) => {
            console.dir(err);
