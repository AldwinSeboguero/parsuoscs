...
export default {
  ...
  methods: {
     readDataFromAPI() {
      ...
      let pageNumber = page;
      axios
        .get(`/api/v1/clearancerequestscbm?page=` + pageNumber, {
    ...
    searchIt(d) {
      if (d.length > 2) {
        const { page, itemsPerPage } = this.options;
        axios
          .get(`/api/v1/clearancerequestscbm/${d}`)
          .then((res) => {
            this.loading = false;  
      ...
      if (d.length <= 0) {
        axios
          .get(`/api/v1/clearancerequestscbm?page=${d.page}`, {
    ...
    deleteItem(item) {
      ...
      if (decide) {
        axios
          .delete("/api/v1/clearancerequestscbm/" + item.id)
          .then((res) => {
    ...
    approve() {
      ...
      // if (decide) {
        axios
          .post("/api/v1/approveclearancerequestcbm", this.clearanceRequest)
          .then((res) => {
    ...
    deferItem(){
      const index = this.editedIndex;
        axios
          .put("/api/v1/clearancerequestscbm/" + this.clearanceRequest.id, this.deficiency)
          .then((res) => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        const index = this.editedIndex;
        axios
          .put("/api/v1/clearancerequestscbm/" + this.editedItem.id, this.editedItem)
          .then((res) => {
            this.text = "Record Updated Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
        ...
        axios
          .post("/api/v1/clearancerequestscbm", this.editedItem)
          .then((res) => {
