...
export default {
  ...
  methods: {
     readDataFromAPI() {
      ...
      let pageNumber = page;
      axios
        .get(`/api/v1/registrarDeficiencyList?page=` + pageNumber, {
    ...
    searchIt(d) {
      if (d.length > 2) {
        const { page, itemsPerPage } = this.options;
        axios
          .get(`/api/v1/registrarDeficiencyList/${d}`)
          .then((res) => {
            this.loading = false;  
      ...
      if (d.length <= 0) {
        axios
          .get(`/api/v1/registrarDeficiencyList?page=${d.page}`, {
    ...
    deleteItem(item) {
      ...
      if (decide) {
        axios
          .delete("/api/v1/registrarDeficiencyList/" + item.id)
          .then((res) => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        axios
          .put("/api/v1/registrarDeficiencyList/" + this.editedItem.id, this.editedItem)
          .then((res) => {
        ...
        axios
          .post("/api/v1/registrarDeficiencyList", this.editedItem)
          .then((res) => {
